...
#####    Forked from original Zabbix nodemonitor.sh script created by Stakezone
...
binDir="$HOME/.local/share/solana/install/active_release/bin/" # auto detection of the solana binary directory can fail or an alternative custom installation is preferred, in case insert like $HOME/solana/target/release
...
if [ -z $rpcURL ]; then
   ...
   if [ -z $rpcPort ]; then
      echo "nodemonitor,pubkey=$identityPubkey status=4 $now"
      exit 1
   fi
...
if [[ ((-n "$currentValidatorInfo" || "$delinquentValidatorInfo")) ]] || [[ ("$validatorBlockTimeTest" -eq "1") ]]; then
   ...
   if [ -n "$delinquentValidatorInfo" ]; then
      ...
      #   logentry="$logentry,
      echo "sol_leaderSlots $leaderSlots" >/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_skippedSlots $skippedSlots" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_pctSkipped $pctSkipped" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_pctTotSkipped $pctTotSkipped" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_pctSkippedDelta $pctSkippedDelta" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_pctTotDelinquent $pctTotDelinquent" >>/var/lib/node_exporter/textfile_collector/sol.prom
      #   echo "sol_version $version" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_pctNewerVersions $pctNewerVersions" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_commission $commission" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_activatedStake $activatedStake" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_credits $credits" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_solanaPrice $solanaPrice" >>/var/lib/node_exporter/textfile_collector/sol.prom
   else status=2; fi
   if [ "$additionalInfo" == "on" ]; then
      ...
      #write to file
      echo "sol_validatorBalance $validatorBalance" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_validatorVoteBalance $validatorVoteBalance" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_nodes $nodes" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_epoch $epoch" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_pctEpochElapsed $pctEpochElapsed" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_validatorCreditsCurrent $validatorCreditsCurrent" >>/var/lib/node_exporter/textfile_collector/sol.prom
      echo "sol_status $status" >>/var/lib/node_exporter/textfile_collector/sol.prom
